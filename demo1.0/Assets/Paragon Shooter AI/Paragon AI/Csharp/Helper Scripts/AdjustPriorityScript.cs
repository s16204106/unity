...
public class AdjustPriorityScript : MonoBehaviour {
	...
	private ParagonAI.TargetScript targetScript;
	...
	void UpdatePriority()
		{
			...
			Transform[] targetsToCycle = ParagonAI.ControllerScript.currentController.GetCurrentAIsWithIDs(teamNumbersToLookFor);
			...
			if(newPriority > 0)
				{
					ParagonAI.ControllerScript.currentController.UpdateAllEnemiesEnemyLists();
				}
		}
