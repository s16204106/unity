...
public class GamePlayController : MonoBehaviourPun, IPunObservable
{
            ...
            for (int x = 0; x < Map.hexMapSizeX; x++)
            {
                for (int z = 0; z < Map.hexMapSizeZ / 2; z++)
                {
                    if (gridChampionsArray[x, z] != null)
                    {
                        ...
                        {
                            GameObject temp = PhotonNetwork.Instantiate(targetState.prefab.name, camp.GetComponent<Transform>().position, camp.GetComponent<Transform>().rotation);
    ...
    IEnumerator DelayChangeState(float t,GameStage gs)
    {
        ...
        {
            currentGold += 6;
            photonView.RPC("UpdatePlayer", RpcTarget.All, playerID, currentHP, currentGold);
            //延时t秒
            yield return new WaitForSeconds(t);
    ...
    [PunRPC]
    private void UpdatePlayer(int id, int health ,int gold)
    {
        for(int i=0;i<playerList.Length;i++)
        {
    ...
    public bool BuyChampionFromShop(Champion champion)
    {
        ...
        //实例化
        GameObject championPrefab = PhotonNetwork.Instantiate(champion.prefab.name, map.ownInventoryGridPositions[emptyIndex], Quaternion.Euler(0, cameraBehavior.point[0].transform.eulerAngles.y, 0));
